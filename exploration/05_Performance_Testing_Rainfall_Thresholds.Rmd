...
p_approach1 <- tbl_performance_overall %>% 
    imap(\(tbl_plot,regime_name){
        plot_performance_metrics(df=tbl_plot,
                         governorate=NULL,
                         pseudo_log=F,
...
Limitations - the problem is that when aggregating dates are consecutive -- let's cluster dates for hajjah an dmarib
```{r}

tar_load(rainfall_impact_tbl)
# lets look at just marib nd hajja
area_lvl_roi <- gov_area_rainfall_impact_tbl %>% 
    filter(governorate_name %in% c("Hajjah","Marib"))

# pull dates of each on there own
area_lvl_event_dates <- split(area_lvl_roi,area_lvl_roi$governorate_name) %>% 
    map(\(df)
        {
           dates <-  df %>% 
            filter(fevent) %>% 
            pull(date)
            # need to multipl year(date) by 100 or 1000 so that 2021 & 2022 are separated
           (year(dates)*100)+yday(dates)
        }
    )
yday("2021-07-24")
2021+205
2022+205
#calculate distance between event dates and then create hclust objects
date_hclust <- area_lvl_event_dates %>% 
    map(
        ~dist(.x) %>% 
            hclust(method="average")
        )

# let's manually understand the grouping and then we can just tell cuttree the number of clusters
area_lvl_roi_events <- area_lvl_roi %>% 
    filter(fevent) %>% 
    print(n=nrow(.))

# ok we want 4 for hajjah looks like probably 4 OR 5
grouping_vectors<- date_hclust %>% 
    map2(.y = c(4,4),
         \(hclust,num_groups){
             cutree(hclust, k=num_groups)
         }
         )
area_lvl_roi_events_dt_grpd <- area_lvl_roi_events %>% 
    mutate(
        date_events_num = unlist(area_lvl_event_dates),
       date_groups= paste0(governorate_name,"_",unlist(grouping_vectors)),.after=date
    ) 

# just viz grouping
area_lvl_roi_events_dt_grpd%>% 
    select(1:4) %>% 
    print(n=nrow(.)) 

area_lvl_roi_events_dt_grpd %>% 
        select(date, date_groups, precip_roll5=precip_roll5_mean) %>% 
    print(n=15)
    

# summarise - for now we go with max precip values, and max date per group
area_lvl_roi_events_summarised <- area_lvl_roi_events_dt_grpd %>% 
    group_by(governorate_name, date_groups) %>% 
    summarise(
        # median_date = median(date),
        date= max(date),
        across(starts_with("precip_"),~max(.x,na.rm=T),.names = "{.col}"),.groups="drop"
        )
area_lvl_roi_events_summarised %>% 
    select(date, date_groups, precip_roll5=precip_roll5_mean)
    glimpse()
# ok so we join back to original impact table by date? 
area_lvl_roi %>%
    select(-fevent) %>% 
    left_join(area_lvl_roi_events_summarised) %>% glimpse() 
    mutate(fevent = ifelse(!is.na(date_groups)))


```

going to try clustering based on spatial coordinates and datestamp together. For this we need coordinates
```{r}
    ...
    event_uid %>% 
        map_dfr( \(uid){
            ...
            site_dates_centered <- df_w_event_uid %>% 
                filter(site_id ==df_event$site_id,
                       date %in% dates_incl) %>% 
...
distributions_centered<- c(3,5,10,15) %>% 
    map(\(day){
        event_rainfall_distributions(df =rainfall_impact_marib_hajjah,
                             day_window = day )
    }
...
plot_regime_distribution_ridges <-  function(df,regime_rgx="^precip_.+\\d$"){
    df_regime_long <- df %>% 
    pivot_longer(cols = matches(regime_rgx),
...
return_yearly_max <-  function(df,agg_by="mean"){
    if(agg_by=="mean") {
        df_agg_across_sites <- df %>% 
            group_by(governorate_name,date) %>% 
            summarise(
                across(starts_with("precip_"),~mean(.x,na.rm=T)),
                .groups="drop"
            )
        yr_max <- df_agg_across_sites %>% 
            group_by(governorate_name, year= year(date)) %>% 
            summarise(
                across(.cols=starts_with("precip_"),~max(.x,na.rm=T) ),
                .groups="drop"
            )
    ...
    if(agg_by =="max"){
        ...
        df_agg_across_sites <- df %>% 
            group_by(governorate_name,date) %>% 
            summarise(
                across(.cols=starts_with("precip_"),~max(.x) ),
                .groups="drop"
            )
        yr_max <- df_agg_across_sites %>% 
            group_by(governorate_name, year= year(date)) %>% 
...
return_yearly_max(df = site_rainfall_historical,agg_by = "mean" )
calc_rps <-  function(df, agg_by,rps = c(2,3,4,5,10)){
    yearly_max <- return_yearly_max(df = site_rainfall_historical,agg_by = agg_by )
    marib_ARI_gev_roll10 <- fevd(yearly_max_marib$precip_roll10, type = "GEV")
    
}


    yearly_max_marib <- yearly_max_by_gov %>% 
    filter(governorate_name=="Marib")
marib_ARI_gev_roll10 <- fevd(yearly_max_marib$precip_roll10, type = "GEV")
marib_ARI_gev_roll5 <- fevd(yearly_max_marib$precip_roll5, type = "GEV")
marib_ARI_gev_roll3 <- fevd(yearly_max_marib$precip_roll3, type = "GEV")
marib_ARI_gev_daily <- fevd(yearly_max_marib$precip_daily, type = "GEV")


rps_roll10 <- return.level(x = marib_ARI_gev_roll10, 
                       return.period = c(2,3,4,5,10),
                       do.ci = TRUE, alpha = 0.05)
rps_roll5 <- return.level(x = marib_ARI_gev_roll5, 
                       return.period = c(2,3,4,5,10),
                       do.ci = TRUE, alpha = 0.05)
rps_roll3 <- return.level(x = marib_ARI_gev_roll3, 
                       return.period = c(2,3,4,5,10),
                       do.ci = TRUE, alpha = 0.05)
rps_daily<- return.level(x = marib_ARI_gev_daily, 
                       return.period = c(2,3,4,5,10),
                       do.ci = TRUE, alpha = 0.05)


ck <- site_rainfall_historical %>% 
    group_by(governorate_name,date) %>% 
    summarise(
        across(starts_with("precip_"),~mean(.x,na.rm=T)),
        .groups="drop"
...
yearly_max_marib_calc2 <- ck2 %>% 
    filter(governorate_name =="Marib")
marib_ARI_gev_roll10_2 <- fevd(yearly_max_marib_calc2$precip_roll10, type = "GEV")
rps_roll10 <- return.level(x = marib_ARI_gev_roll10_2, 
                       return.period = c(2,3,4,5,10),
                       do.ci = TRUE, alpha = 0.05)
