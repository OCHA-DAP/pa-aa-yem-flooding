...
rainfall_impact_marib <- rainfall_impact_tbl %>%
                      filter(governorate_name %in% c("Marib"))
rainfall_impact_hajjah <- rainfall_impact_tbl %>%
                      filter(governorate_name %in% c("Hajjah"))

marib_event_dates<- rainfall_impact_marib %>% 
    filter(fevent) %>% 
    pull(date) %>% 
    unique()
hajjah_event_dates<- rainfall_impact_hajjah %>% 
    filter(fevent) %>% 
    pull(date) %>% 
    unique()

for(i in which(rainfall_impact_marib$fevent)){
    d <- rainfall_impact_marib %>% 
        slice(i)
}

rainfall_impact_temp_filtered<- marib_event_dates %>% 
    map_dfr(\(dt){
        start_dt <- dt-10
        end_dt <- dt+10

        rainfall_impact_marib %>% 
            group_by(governorate_name,site_id) %>% 
            filter(date>=start_dt,date<=end_dt) 
    }
    )
rainfall_impact_temp_filtered %>% 
    ggplot(aes(x= precip_roll10))+
    geom_histogram()+
    facet_wrap(~governorate_name)

rainfall_impact_filt %>% 
    filter(fevent) %>% 
    mutate(f_roll= floor(precip_roll10)) %>% 
    count(f_roll) %>% 
    arrange(desc(n))
