...
adjust threshold to correct for forecast bias?

The simplest is just to take the mean of the leadtimes this:
```{r}
gefs_rps %>% 
    filter(leadtime %in% c(3:7),
           RP==2) %>% 
...
```{r}
gefs_leadtime_mean <- gefs_zonal_rolled %>% 
    filter(name =="roll3") %>% 
    filter(!leadtime %in%c(1:2),
           year(date_forecast_predict)!=2023,
           year(date_forecast_predict)!=2020,
...
zonal_stats_high_risk_hull$roll3 %>% 
    ...
    filter(year(date)!=2021) %>% 
    left_join(
        gefs_leadtime_mean, by=c("date"="date_forecast_predict","governorate_name"="governorate_name")
    ) %>% 
    filter(mean!=0,value!=0) %>% 
