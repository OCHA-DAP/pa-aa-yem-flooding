...
Realtime Simulation {data-orientation=rows}
=====================================  
...
fluidRow("select params" ,
         column(width=3,
                dateInput(inputId="date",label = "Date",value = "2022-07-13")
                ),
         column(width=3,
                selectInput(inputId="cumulation_window",choices = c("Daily"="precip_daily",
                                                    "3 Day"="roll3",
                                                    "5 Day"="roll5",
...
renderPlot({
    ...
    y_axis_label <- convert_window_numeric(input$cumulation_window) %>%
        paste0("Rainfall Accumulation (mm)")
...
Lead Time {data-orientation=rows}
=====================================     
   

### Lead Times

```{r test2, echo=FALSE, out.width="100%", height="500px"}
fluidRow(
         column(width=4,
                selectInput(inputId="cumulation_window2",choices = c("Daily"="precip_daily",
                                                    "3 Day"="roll3",
                                                    "5 Day"="roll5",
                                                    "10 Day"= "roll10"),label = "Rainfall accumulation",selected = "roll3")),
         column(width=4,
                selectInput(inputId="gov2",choices = c("Hajjah","Marib"),label = "Governorate",selected = "Hajjah")),
         column(width=4,
                numericInput(inputId = "thresh2",label = "Threshold",value = 19.8,min = 0,step = 0.1)
                )
...
convert_window_numeric_r2 <- reactive({
    convert_window_numeric(input$cumulation_window2)
})

renderPlot({
    roll_number<- as.numeric(trimws(convert_window_numeric(input$cumulation_window2)))
    rm_leads <- seq(0,(roll_number-1))

    forecast_temp_filt <- forecast_filt2() %>% 
        filter(year(date_forecast_predict)>=2017) %>% 
        filter(!leadtime %in% rm_leads)
    
    hres_gte_thresh_max_lead <- forecast_temp_filt %>% 
        group_by(activation,date_forecast_predict) %>% 
        filter(value>=input$thresh) %>% 
    ...
    p <- forecast_temp_filt %>% 
        ...
        scale_color_manual(values = c("Action"="red","Readiness"="orange"),na.translate=F,drop=F)+
        geom_hline(yintercept = input$thresh2)+
        scale_x_date(
            # breaks = "1 day",date_labels = "%m-%d"
            )+
        # scale_y_continuous(limits = c(-2,top_plot),breaks = seq(0,100,10))+
        labs(y=y_axis_label(),
             title  = "Activations per leadtime"
             )+
        facet_grid(rows = vars(leadtime), scales = "free_y")+
        theme_hdx()+
        theme(
            axis.text.x = element_text(angle=90),
        )
...
```{r uiEraChirpHres, echo=FALSE, out.width="100%", height="500px"}
fluidRow(
         column(width=2,
         ...
         column(width=2,
                selectInput(inputId="trig_freq",label="Trigger can be activaed 1x per",choices=c("Season"="season","10 Days","30 Days"),selected="season")
         ),
         column(width=2,
                selectInput(inputId="forecast_type",label="Forecast:",choices=c("CHIRPS-GEFS"="chirps_gefs","ECMWF - HRES"="hres"),selected="hres")
         )
)
```
...
renderDataTable({
    ...
    tables_merged %>% 
        filter(precip_regime==input$cum_window3,RP==2)
...
renderPlot({
    ...
    # group_by(governorate_name,date_forecast_predict) %>% 
    # summarise(
