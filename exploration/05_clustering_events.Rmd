...
map_pt_clusters(df = cccm_impact_c1,
                date = "date",
                lon = "lon",
                "lat",
...
df_clusters5 <- spatial_pt_clusters(df = cccm_impact_c1,
                date = "date",
                lon = "lon",
                "lat",
                event = "fevent",
                scale = F,
                k = 5)
df_clusters7 <- spatial_pt_clusters(df =cccm_impact_c1,
                                        # roi %>% 
...
performance_clusters_roll10 %>% 
    imap(
        \(summary_class_tbl,nm){
            summary_class_tbl$class_freq %>%  
                ...
                geom_line()+
                geom_line(data= summary_class_tbl$performance_metric,
                          aes(x=threshold, y=value*10,color=metric,group=metric)
                )+
                scale_y_continuous( sec.axis = sec_axis(trans =~.*(1/10), name="ratio"))+
                labs(title = glue("{nm}: Performance Graph - 10 day accumulation"))+
                theme_hdx()
        }
    )
```
