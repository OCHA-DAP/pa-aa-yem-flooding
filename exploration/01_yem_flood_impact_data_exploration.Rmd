...
From this code we learn that there are 356 unique sites, and 260 unique sites with gps coordinates
```{r}
...
all(cccm_new %>% map(~ nrow(.)) %>% unlist() == cccm_dfs %>%
  map(~ nrow(.x)) %>%
  unlist())
...
adm3$admin1Name_en %>%
  unique() %>%
  sort()
to_snake_case(adm3$admin1Name_en) %>%
  unique() %>%
  sort()
...
This portion of code was just run to quickyl query the various files received from CCCM
