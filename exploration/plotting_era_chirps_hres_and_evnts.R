...
merge_rainfall_event_datasets(
  hres = hres_rolled_per_date_gen,
  era = era5_w_rolling,
...
hres_rolled_per_date_gen %>%
  ...
  filter(year(date_forecast_predict) == 2007, month(date_forecast_predict) == 7) %>%
  View()
filter(value > 30)
...
merge_rainfall_event_datasets <- function(hres = hres_rolled_per_date_gen,
  ...
  hres_chirps_era_events <- events_by_date %>%
    left_join(hres_chirps_era) %>%
    mutate(
      precip_regime = precip_regime_label,
      title = plot_label
    )
...
# Lead Time 5
merged_df_lead5 <- merge_rainfall_event_datasets(
  hres = hres_rolled_per_date_gen,
  era = era5_w_rolling,
  chirps = zonal_stats_high_risk_hull,
  events = gov_area_rainfall_impact_tbl,
...
plot_rainfalls_forecast_events(
  df = merged_df_lead5,
  gov = "Hajjah",
  thresh = 19.8,
  look_back = 7,
...
merged_df_lead10 <- merge_rainfall_event_datasets(
  hres = hres_rolled_per_date_gen,
  era = era5_w_rolling,
  chirps = zonal_stats_high_risk_hull,
  events = gov_area_rainfall_impact_tbl,
