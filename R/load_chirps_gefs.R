...
#' @param mask spatial file in wgs84 projection to use for cropping rasters and calculating zonal stats
#' @param write_outputs \code{logical} if T (default) write outputs to gdrive.
#' @param gdrive_dribble `{googledrive}` class object containing all the files the service account has access to
#' @param raster_drive dribble (`{googledrive}` class object) containing the location to store the rasters on the drive 
#' @param zonal_drive dribble (`{googledrive}` class object) containing the location to store the zonal stats csvs.
#' @return
#' If data is not already on gdrive it will download the latest chirps-gefs data and produce:
#' 1. CHIRPS-GEFS raster tifs clipped to mask bounding box.
#' 2. Zonal mean by governorate for each raster as in memory object and csv.
#' If the data is not already available it will return a `NULL` value

conditionally_load_latest_chirps_gefs <- function(
                                           leadtime = 1:10,
                                           mask = roi,
                                           write_outputs = T,
    ...
    meta_tbl <- latest_gefs_metadata(leadtime=leadtime)
    cond_already_downloaded<- all(meta_tbl$drive_fname %in% gdrive_dribble$name)
    if(cond_already_downloaded){
        cat("data already downloaded - no update necessary")
        ret <- NULL
    }
    if(!cond_already_downloaded){
