...
# same records - hopefully the new has coordinates
all(cccm_new %>% map(~nrow(.)) %>% unlist() == cccm_dfs %>% map(~nrow(.x)) %>% unlist())
...
# basically a master list
cccm_new$`ML- Flooding Available data` %>% 
    count(site_id) %>% summarise(mean_n=mean(n))

# here are flood reports - same idp sites show up more than 1x. Max # of times is 8 which
# indicates 1 or 2 sites are not dominating reports... but it could be a specific admin
cccm_new$`CCCM FLOOD REPORT IN IDP SITES` %>% 
...
# this has 14 less than other master-- let's use it for now.. 
cm_master_flood_db <- cccm_dfs$`ML- Flooding Available data` 

lat_lon_lookup <- cm_master_flood_db %>% 
    select(site_id, longitude = available_coordinates_longitude, latitude = available_coordinates_latitude)
...
just reported between 2021-06-12& 2022-09-26
```{r}
...
I wonder is this just proportional to the # of sites CCCM is monitoring per governorate

```{r}
idp_sites_per_gov_tbl <- cm_master_flood_db %>% 
    group_by(governorate_name) %>%
    summarise(
        num_records=n()
        ) %>%
...
bind_rows(idp_sites_per_gov_tbl,reported_floods_tbl) %>% 
    ggplot(aes(x= reorder(governorate_name,num_records),
               y= num_records,fill=source))+
    geom_col(position="dodge")+
    # geom_text(aes(label=num_records),hjust=-1.2)+
    # scale_y_continuous(breaks = seq(0,175, by=25), limits = c(0,150))+
    coord_flip()+
    labs(title= "CCCM Flood Report Data: # of Sites vs # Flood Reports",
