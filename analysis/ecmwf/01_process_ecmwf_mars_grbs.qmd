...
The ECMWF MARS data appears to be in a very odd format which is proving quite difficult. It may be possibl to get it in a better format in the future, but for now we will wrangle it- yeeehaw
...
For this notebook I will first attempt/create the process on a single file and then attempt to functionalize and iterate for all the files

Load a single file the way we would expect with `{xarray}` 
```{python}
...
ok above is looking like garbage  -- lets try a better approach
...
Ok let's functionalize it now and iterate through all files
```{python}
def grid_from_xarray_points(dataset,param,step, res= 0.1):
    data_sub = dataset[param].isel(step=step)
    points = list(zip(data_sub.longitude.values, data_sub.latitude.values))
    vals = data_sub.values

    xRange = data_sub.arange(data_sub.longitude.min(), data_sub.longitude.max()+ res,res)
    yRange = data_sub.arange(data_sub.latitude.min(), data_sub.latitude.max()+ res, res)
    gridX,gridY = np.meshgrid(xRange, yRange)
    grid_ecmwf = griddata(points, forecast_vals, (gridX, gridY),method='linear')
    transform = Affine.translation(gridX[0][0]-res/2, gridY[0][0]-res/2)*Affine.scale(res,res)
    rasterCrs = CRS.from_epsg(4326)

def write_grid_to_tiff(dataset,param,step, res= 0.1):
    data_sub = dataset[param].isel(step=step)
    points = list(zip(data_sub.longitude.values, data_sub.latitude.values))
    vals = data_sub.values
